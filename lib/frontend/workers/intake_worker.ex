defmodule Frontend.Workers.IntakeWorker do
  ...
  def perform(%Oban.Job{args: %{"source_video_id" => source_video_id}}) do
    ...
    with {:ok, source_video} <- Clips.get_source_video(source_video_id),
      ...
      Logger.info("IntakeWorker: Running PythonRunner for source_video_id: #{source_video_id}, URL: #{source_video.original_url}")
      ...
      case PythonRunner.run("intake", %{
        ...
        {:ok, result} ->
          Logger.info("IntakeWorker: PythonRunner succeeded for source_video_id: #{source_video_id}, result: #{inspect(result)}")
        ...
        {:error, reason} ->
          ...
          Logger.error("IntakeWorker: PythonRunner failed for source_video_id: #{source_video_id}, reason: #{inspect(reason)}")
          error_message = inspect(reason)
          Clips.update_source_video(source_video, %{ingest_state: "download_failed", last_error: error_message})
          {:error, reason}
      end
    else
      # Handles errors from the `with` statement (e.g., video not found or already processed)
      {:error, :already_processed} ->
        Logger.info("IntakeWorker: source_video_id #{source_video_id} already processed, skipping")
        :ok # Gracefully exit, not an error.
      {:error, reason} ->
        Logger.error("IntakeWorker: Error getting source video #{source_video_id}: #{inspect(reason)}")
        {:error, reason} # Propagate other errors for Oban to retry
    end
  end
