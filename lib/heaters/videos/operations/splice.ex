defmodule Heaters.Videos.Operations.Splice do
  ...
  defp perform_splice_workflow(source_video, temp_dir, opts, start_time) do
    with {:ok, local_video_path} <- download_source_video(source_video, temp_dir),
      ...
      Logger.info(
        "Native Splice: Successfully processed #{length(clips_data)} clips in #{duration_ms}ms"
      )
  ...
  defp run_scene_detection(video_path, opts) do
    ...
    case SceneDetector.detect_scenes(video_path, detection_opts) do
      {:ok, result} ->
        Logger.info(
          "Native Splice: Scene detection completed: #{length(result.scenes)} scenes found"
        )
  ...
  defp process_single_scene(
    ...
    case S3Adapter.head_object(s3_clip_key) do
      {:ok, metadata} ->
        Logger.info(
          "Native Splice: Clip #{clip_identifier} already exists in S3 (#{metadata.content_length} bytes), skipping extraction"
        )
      ...
      {:error, reason} ->
        Logger.warning(
          "Native Splice: Failed to check S3 existence for #{s3_clip_key}: #{inspect(reason)}, proceeding with extraction"
        )
  ...
  defp extract_and_upload_clip(
    ...
    Logger.debug(
      "Native Splice: Extracting clip #{clip_identifier}: #{scene.start_time_seconds}s - #{scene.end_time_seconds}s"
    )
    ...
    with {:ok, file_size} <-
      ...
      Logger.info(
        "Native Splice: Successfully created and uploaded clip: #{clip_identifier} (#{file_size} bytes)"
      )
      ...
      {:error, reason} ->
        Logger.error(
          "Native Splice: Failed to extract/upload clip #{clip_identifier}: #{inspect(reason)}"
        )
