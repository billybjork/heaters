defmodule Heaters.Videos.FinalizeCache.Worker do
  @moduledoc """
  Worker for finalizing cached temporary files to S3 storage.
  ...
  1. Check if source video needs cache finalization
  ...
  5. Mark finalization as complete

  ## State Management

  - **Input**: Source videos with scene detection complete but cache not finalized
  ...
  - **Idempotency**: Skip if already finalized or no cached files found
  ...
  - **Storage**: Finalizes temporary files to permanent S3 storage
  """
  ...
  alias Heaters.Infrastructure.CacheArgs
  require Logger
  ...
  defp handle_finalization_work(%{"source_video_id" => source_video_id}) do
    Logger.info(
      "FinalizeCacheWorker: Starting cache finalization for source_video_id: #{source_video_id}"
    )

    with {:ok, source_video} <- Queries.get_source_video(source_video_id) do
      handle_finalization(source_video)
    else
      {:error, :not_found} ->
  ...
  defp handle_finalization(%SourceVideo{} = source_video) do
    # Check if finalization is needed
    case needs_finalization?(source_video) do
      true ->
        Logger.info("FinalizeCacheWorker: Starting finalization for video #{source_video.id}")
  ...
  defp needs_finalization?(source_video) do
    # Video needs finalization if:
  ...
  defp run_finalization_task(source_video) do
    ...
    if length(all_keys) > 0 do
      Logger.info(
        "FinalizeCacheWorker: Finalizing #{length(all_keys)} potential cached files for video #{source_video.id}"
      )

      # Use CacheArgs to finalize all cached files
  ...
  defp mark_finalization_complete(source_video) do
    ...
    case Heaters.Repo.update(changeset) do
      {:ok, _updated_video} ->
        Logger.info(
