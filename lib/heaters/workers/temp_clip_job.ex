defmodule Heaters.Workers.TempClipJob do
  @moduledoc """
  ...
  def perform(%Oban.Job{args: %{"clip_id" => clip_id}}) do
    ...
    clip =
      Heaters.Repo.get!(Heaters.Clips.Clip, clip_id)
      |> Heaters.Repo.preload(:source_video)

    case Heaters.Clips.TempClip.build(clip) do
      {:ok, "/temp/" <> filename} ->
        Logger.info("TempClipJob: Successfully generated temp clip #{filename}")

        # Broadcast to LiveView that temp file is ready
        broadcast_result = HeatersWeb.Endpoint.broadcast("clips:#{clip_id}", "temp_ready", %{
          path: "/temp/#{filename}",  
          clip_id: clip_id
        })
        Logger.info("TempClipJob: Broadcast result: #{inspect(broadcast_result)} for clips:#{clip_id}")
