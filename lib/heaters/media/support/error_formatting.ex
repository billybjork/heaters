defmodule Heaters.Media.Support.ErrorFormatting do
  @moduledoc """
  ...
  def format_validation_context(operation, context) do
    ...
    case context do
      ...
      :state_transition_check ->
        "#{operation_name} operation state transition validation failed"

      _ ->
        "#{operation_name} operation #{context} validation failed"
    end
  end

  @doc """
  Format any error reason into a string message.

  This is a general-purpose error formatter that handles various error types.
  """
  @spec format_error(any()) :: String.t()
  def format_error(error_reason) when is_binary(error_reason), do: error_reason
  def format_error(error_reason), do: inspect(error_reason)
end
