defmodule Heaters.Media.VirtualClip.Operations do
  ...
  def create_virtual_clips_from_cut_points(source_video_id, cut_points, metadata \\ %{}) do
    Logger.info(
      "VirtualClip.Operations: Creating #{length(cut_points)} virtual clips for source_video_id: #{source_video_id}"
    )
    ...
    case get_existing_virtual_clips(source_video_id) do
      [] ->
        ...
        case Validation.validate_cut_points(cut_points) do
          :ok ->
            create_clips_from_validated_cut_points(source_video_id, cut_points, metadata)

          {:error, reason} ->
            Logger.error("VirtualClip.Operations: Cut points validation failed: #{reason}")
            {:error, reason}
        end

      existing_clips ->
        Logger.info(
          "VirtualClip.Operations: Found #{length(existing_clips)} existing virtual clips for source_video_id: #{source_video_id}"
        )
  ...
  @spec get_existing_virtual_clips(integer()) :: [Clip.t()]
