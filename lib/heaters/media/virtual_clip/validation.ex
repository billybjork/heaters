defmodule Heaters.Media.VirtualClip.Validation do
  @moduledoc """
  ...
  def validate_mece_for_source_video(source_video_id) do
    virtual_clips = VirtualClip.get_virtual_clips_for_source(source_video_id)
  ...
  @spec validate_cut_points(list()) :: :ok | {:error, String.t()}
  def validate_cut_points(cut_points) when is_list(cut_points) do
    # Basic validation - ensure cut points are properly formatted
    case Enum.all?(cut_points, &valid_cut_point_format?/1) do
      true -> :ok
      false -> {:error, "Invalid cut point format"}
    end
  end

  @spec valid_cut_point_format?(map()) :: boolean()
  defp valid_cut_point_format?(cut_point) do
    required_fields = ["start_frame", "end_frame", "start_time_seconds", "end_time_seconds"]

    case cut_point do
      %{} = cp when is_map(cp) ->
        Enum.all?(required_fields, &Map.has_key?(cp, &1))
